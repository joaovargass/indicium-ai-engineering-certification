...
def _sql_pool_resume() -> bool:
    ...
    try:
        out = subprocess.run(
            ["az", "synapse", "sql", "pool", "show", "--name", pool, "--workspace-name", ws, "-g", rg, "--query", "status", "-o", "tsv"],
        ...
        if status != "paused":
            return False
        r = subprocess.run(
            ["az", "synapse", "sql", "pool", "resume", "--name", pool, "--workspace-name", ws, "-g", rg],
...
def _sql_pool_pause() -> None:
    ...
    try:
        ...
        if status != "online":
            return
        r = subprocess.run(
            ["az", "synapse", "sql", "pool", "pause", "--name", pool, "--workspace-name", ws, "-g", rg],
            capture_output=True, text=True, timeout=600,
        )
        if r.returncode != 0:
...
def run_incremental_elt(full_refresh: bool | None = None) -> str:
    ...
    try:
        ...
        # Stage: upload - upload deltas to Azure
        try:
        ...
        # Stage: download_deltas - download unprocessed deltas
        try:
        ...
        if combined_df is not None and len(combined_df) > 0:
            ...
            # Stage: load_dw - load to data warehouse
            try:
        ...
        # Stage: cache - update local cache
        ...
        # Stage: estado - update extraction date (only on full success)
        try:
